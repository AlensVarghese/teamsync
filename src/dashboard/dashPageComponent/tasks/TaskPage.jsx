...
const TaskPage = () => {
  ...
  const handleToggleCompleted = async (task) => {
    try {
      const toggledTask = await toggleTaskCompletion(task._id);
      setProjects((prevProjects) =>
        prevProjects.map((proj) =>
          proj._id === task.projectId
            ? {
                ...proj,
                tasks: (proj.tasks || []).map((entry) =>
                  entry._id === toggledTask._id ? toggledTask : entry
                ),
              }
            : proj
        )
  ...
  const userProjects = projects.filter((project) => {
    const coordinatorId =
      typeof project.coordinatorId === "object"
        ? project.coordinatorId?._id
  ...
  const manageableProjects = userProjects.filter((project) => {
    const coordinatorId =
      typeof project.coordinatorId === "object"
        ? project.coordinatorId?._id
        : project.coordinatorId;

    return user?.role === "Admin" || coordinatorId === currentUser._id;
  });
  ...
  const tasks = userProjects.flatMap((proj) =>
    (proj.tasks || [])
      .filter((task) => {
        ...
        const coordinatorId =
          typeof proj.coordinatorId === "object"
            ? proj.coordinatorId?._id
            : proj.coordinatorId;
        if (user?.role === "Coordinator" && coordinatorId === currentUser._id) {
          return true;
        }
  ...
  return (
    <div className="">
      <div className="flex justify-between items-center mb-4">
        ...
        {(user?.role === 'Admin' || manageableProjects.length > 0) && (
          <Button
              ...
              <tr key={task._id}>
                ...
                <td className="border p-2">
                  <div className="flex items-center justify-center gap-2 h-full">
                    {(user?.role === 'Admin' ||
                      manageableProjects.some((project) => project._id === task.projectId)) && (
                    <>
                        <TooltipButton
      ...
      {openAddTaskModal && (user?.role === 'Admin' || manageableProjects.length > 0) && (
      ...
      {editingTask &&
        (user?.role === 'Admin' ||
          manageableProjects.some((project) => project._id === editingTask.projectId)) &&(
        <div className="fixed inset-0 flex items-center justify-center z-50">
          <div
            className="fixed inset-0 bg-black opacity-50"
