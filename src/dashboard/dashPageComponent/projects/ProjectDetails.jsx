...
import GlobalAddTaskForm from "../tasks/GlobalAddTaskForm";
    ...
    <div className="w-full relative">
      <div className="w-full flex gap-6 ">
        <div className="flex flex-col gap-4 p-6 rounded-2xl w-2/3 bg-white border border-gray-200 text-customBlack">
          ...
          <div className="flex items-center gap-2">
            <p className="text-sm text-customBlack">Coordinator: </p>
            {project.coordinatorId ? (
              <>
                <UserAvatar email={project.coordinatorId.email} size={32} />
                <p className="text-sm">
                  {project.coordinatorId.email.split("@")[0]}
                </p>
              </>
            ) : (
          ...
          <div className="bg-white rounded-xl p-6 w-[40%] relative">
            ...
            <form onSubmit={handleEditSubmit}>
              ...
              <div className="mb-4">
                ...
                <select
                  value={managedCoordinatorId}
                  onChange={(e) => setManagedCoordinatorId(e.target.value)}
                  className="w-full p-2 border border-gray-300 bg-white text-black rounded"
                >
                  <option value="">No coordinator</option>
                  {availableMembers.map((member) => (
                    <option key={member._id} value={member._id}>
                      {member.email.split("@")[0]} ({member.role})
                    </option>
                  ))}
                </select>
              </div>
      ...
      {openAddTaskModal && canManageTasks && (
        <div className="fixed inset-0 flex items-center justify-center z-20 bg-black bg-opacity-50">
          <div className="bg-white rounded-xl p-6 w-10/12 md:w-[40%] relative">
            <button
