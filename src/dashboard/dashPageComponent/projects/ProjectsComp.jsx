...
const ProjectsComp = () => {
  ...
  const [newProjectCoordinatorId, setNewProjectCoordinatorId] = useState("");
  const [users, setUsers] = useState([]); // Define users state
  const [availableMembers, setAvailableMembers] = useState([]);

  const coordinatorOptions = useMemo(
    () => users.filter((entry) => entry.email !== currentUserEmail),
  ...
  const handleAddProjectSubmit = async (e) => {
    ...
    const newProject = {
      ...
      coordinatorId: newProjectCoordinatorId || null,
    };
  ...
  const isProjectCoordinator = (project) => {
    const coordinatorId =
      typeof project.coordinatorId === "object"
        ? project.coordinatorId?._id
        : project.coordinatorId;

    return coordinatorId === localStorage.getItem("userId");
  };
  if (filter === "Archived") {
          ...
          <div className="bg-white rounded-xl p-6 w-[40%] relative">
            ...
            <form onSubmit={handleAddProjectSubmit}>
              ...
              <div className="mb-4">
                ...
                <select
                  value={newProjectCoordinatorId}
                  onChange={(e) => setNewProjectCoordinatorId(e.target.value)}
                  className="w-full p-2 border bg-white text-black border-gray-300 rounded"
                >
                  <option value="">No coordinator</option>
                  {coordinatorOptions.map((entry) => (
                    <option key={entry._id} value={entry._id}>
                      {getUsernameFromEmail(entry.email)} ({entry.role})
                    </option>
                  ))}
                </select>
              </div>
